...
class MysqlMetaSync:
    ...
    def rename_table(self, database, old_table_name, new_table_name):
        ...
        try:
            ...
            mysql_process_out, mysql_process_err = mysql_process.communicate()

            if mysql_process.returncode != 0:
                self.logger(self.level["error"],
                            "failed to get create table [{new_table_name}] sql".format(new_table_name=new_table_name))
                self.logger(self.level["error"], mysql_process_err.strip())
                raise BaseException
            else:
                self.logger(self.level["info"],
                            "finish to get create table [{new_table_name}] sql".format(new_table_name=new_table_name))
        except BaseException as e:
            msg = traceback.format_exc()
            self.logger(self.level["error"],
                        "failed to get create table [{new_table_name}] sql".format(new_table_name=new_table_name))
        ...
        if mysql_process_out.lower().find("create table") != -1:
            ...
            for host in self.hosts:
                try:
                    self.logger(self.level["info"],
                                "begin to create new table [{new_table_name}] in {host}"
                                .format(new_table_name=new_table_name, host=host))
                    create_new_table_sql_command = [
                        self.install_dir + '/bin/mysql',
                        '-h', host,
                        '-D', database,
                        '-P', self.port,
                        '-u', self.mysql_user,
                        '-p' + CryptoUtil.decrypt(self.mysql_password),
                        '-e', create_new_table_sql
                    ]
                    create_new_table_process = subprocess.Popen(
                        create_new_table_sql_command,
                        shell=False,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE
                    )
                    cnt_out, cnt_err = create_new_table_process.communicate()
                    if create_new_table_process.returncode != 0:
                        self.logger(self.level["error"],
                                    "failed to create new table [{new_table_name}] in host [{host}], sql info: {sql}"
                                    .format(new_table_name=new_table_name, host=host, sql=create_new_table_sql))
                        self.logger(self.level["error"], cnt_err.strip())
                    else:
                        self.logger(self.level["info"],
                                    "finish to create new table [{new_table_name}] in host [{host}], sql info: {sql}"
                                    .format(new_table_name=new_table_name, host=host, sql=create_new_table_sql))

                    self.logger(self.level["info"],
                                "begin to drop old table [{old_table_name}] in {host}"
                                .format(old_table_name=old_table_name, host=host))
                    drop_old_table_sql = "drop table " + old_table_name
                    drop_old_table_sql_command = [
                        self.install_dir + '/bin/mysql',
                        '-h', host,
                        '-D', database,
                        '-P', self.port,
                        '-u', self.mysql_user,
                        '-p' + CryptoUtil.decrypt(self.mysql_password),
                        '-e', drop_old_table_sql
                    ]
                    drop_old_table_process = subprocess.Popen(
                        drop_old_table_sql_command,
                        shell=False,
                        stdout=subprocess.PIPE,
                        stderr=subprocess.PIPE
                    )
                    dot_out, dot_err = drop_old_table_process.communicate()
                    if drop_old_table_process.returncode != 0:
                        self.logger(self.level["error"],
                                    "failed to drop old table [{old_table_name}] in host [{host}], sql info: {sql}"
                                    .format(old_table_name=old_table_name, host=host, sql=drop_old_table_sql))
                        self.logger(self.level["error"], dot_out.strip())
                    else:
                        self.logger(self.level["info"],
                                    "finish to drop old table [{old_table_name}] in host [{host}], sql info: {sql}"
                                    .format(old_table_name=old_table_name, host=host, sql=drop_old_table_sql))
                except subprocess.CalledProcessError:
                    msg = traceback.format_exc()
                    self.logger(self.level["error"],
                                "failed to rename table [{old_table_name}] to [{new_table_name}] in host [{host}]"
                                .format(old_table_name=old_table_name, new_table_name=new_table_name, host=host))
                    self.logger(self.level["error"], msg)
