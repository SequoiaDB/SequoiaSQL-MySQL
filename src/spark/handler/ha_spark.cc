...
static handler *spark_create_handler(handlerton *hton,
                                       TABLE_SHARE *table,
                                       MEM_ROOT *mem_root);
...
static const char *ha_spark_exts[] = {
  NullS
};
  ...
  if (field->maybe_null()) {
    ...
    if (is_select) {
      ha_thd()->raise_warning_printf(ER_WARN_NULL_TO_NOTNULL,
                                     field->field_name,
                     ha_thd()->get_stmt_da()->current_row_for_condition());
    }
    field->set_default();
  }
}
...
int ha_spark::convert_row_to_mysql_row (uchar *record) {
  ...
  for (Field **fields = table->field; *fields; fields++) {
      ...
      for (n_col = 1; n_col <= n_columns; n_col ++) {
        SQLLEN data_type;
        ok_stmt(ret, m_stmt, SQLColAttribute(m_stmt, n_col,
                                             SQL_DESC_BASE_COLUMN_NAME,
                                             col_name, MAX_NAME_LEN, NULL, NULL));
        if (0 == strcmp((const char*)col_name, field->field_name)) {
          ok_stmt(ret, m_stmt, SQLColAttribute(m_stmt, n_col, SQL_DESC_TYPE,
                                          NULL, 0, NULL, &data_type));
          switch(data_type) {
            /* SQL data type codes */
          /* SQL data type codes */
          case SQL_CHAR:
          ...
          case SQL_NUMERIC: {
             ...
             ok_stmt(ret, m_stmt, SQLGetData(m_stmt, n_col, SQL_C_CHAR,
                                       (SQLPOINTER)col_value, sizeof(col_value),
                                       &len_or_null));
            if (SQL_NULL_DATA == len_or_null) {
              set_field_null(field, is_select);
              continue;
            }
            field->store((const char*)col_value, strlen((const char*)col_value),
                         &SPK_DEFAULT_CHARSET);
            break;
          }
          case SQL_TINYINT: {
            SQLSCHAR value = 0;
            ok_stmt(ret, m_stmt, SQLGetData(m_stmt, n_col, SQL_C_TINYINT,
                                            (SQLPOINTER)&value,
          ...
          case SQL_SMALLINT: {
            SQLSMALLINT value = 0;
            ok_stmt(ret, m_stmt, SQLGetData(m_stmt, n_col, SQL_C_SHORT,
                                            (SQLPOINTER)&value,
                                            sizeof(value), &len_or_null));
            if (SQL_NULL_DATA == len_or_null) {
              set_field_null(field, is_select);
          ...
          case SQL_INTEGER: {
            SQLINTEGER value = 0;
            ok_stmt(ret, m_stmt, SQLGetData(m_stmt, n_col, SQL_C_LONG,
                                            (SQLPOINTER)&value,
                                            sizeof(value), &len_or_null));
            if (SQL_NULL_DATA == len_or_null) {
              set_field_null(field, is_select);
          ...
          case SQL_BIGINT: {
            SQLBIGINT value = 0;
            ok_stmt(ret, m_stmt, SQLGetData(m_stmt, n_col, SQL_C_SBIGINT,
                                            (SQLPOINTER)&value,
                                            sizeof(value), &len_or_null));
            if (SQL_NULL_DATA == len_or_null) {
              set_field_null(field, is_select);
          ...
          case SQL_REAL: {
            SQLREAL value = 0;
            ok_stmt(ret, m_stmt, SQLGetData(m_stmt, n_col, SQL_C_FLOAT,
                                            (SQLPOINTER)&value,
                                            sizeof(value), &len_or_null));
            if (SQL_NULL_DATA == len_or_null) {
              set_field_null(field, is_select);
...
int ha_spark::rnd_next(uchar *buf)
{
  ...
  if (first_read) {
    if (ha_thd()->variables.sdb_sql_pushdown &&
        ha_thd()->prepare_step == ha_thd()->sdb_join_exec_step &&
        ha_thd()->sdb_join_push_down_query_string.length) {

      spk_query_str = ha_thd()->sdb_join_push_down_query_string.str;
      ha_thd()->sdb_join_exec_step = ha_thd()->exec_step;

      THD_STAGE_INFO(ha_thd(), stage_exec_sql_in_spk);
      ha_statistic_increment(&SSV::ha_read_rnd_next_count);
    } else {
      spk_query_str = ha_thd()->query().str;
    }
...
THR_LOCK_DATA **ha_spark::store_lock(THD *thd,
                                       THR_LOCK_DATA **to,
...
mysql_declare_plugin(spark)
{
  ...
  &spark_storage_engine,
  "Spark",
  "SequoiaDB Inc",
  "SequoiaDB Spark engine",
  PLUGIN_LICENSE_GPL,
